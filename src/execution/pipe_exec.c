...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/03/08 21:00:12 by tpauvret          #+#    #+#             */
/*   Updated: 2022/03/09 13:37:19 by tpauvret         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../inc/minishell.h"

static int	create_fd_pipe(t_mini *shell, t_process *proc)
{
	int	i;
...
static int	pipe_fork(t_command	*child, t_env **env_list, t_process *proc,
			int *cmd)
{
	int	i;
	...
	{
		if (ft_redirection(child->redirection, proc))
			exit(1);
	}
...
static int	close_fd(t_mini *shell, t_process *proc)
{
	int	i;
	int	status;
